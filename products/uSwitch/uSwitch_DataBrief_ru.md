...
**Внимание! Текущая версия обеспечивает передачу только 4 кодов. Передача 8 кодов планируется в следующей версии системы.**  
...
На каждом из модулей есть контакты **STROBE**, **BIT 0**, **BIT 1**, **BIT 2**. В _передатчике_ это _цифровые входы_, а в _приемнике_ - _цифровые выходы_.
По умолчанию все эти контакты и на приемнике и на передатчике установлены в состояние цифровой единицы (3.3 Вольта). 

Передаваемый код формируется состоянием контактов (пинов) **BIT 0**, **BIT 1** и **BIT 2**, а передача происходит, когда состояние пина **STROBE** переходит из состояния цифровой единицы в стотояние цифрового нуля.

На приемной стороне принятый код вызывает изменение пинов **BIT 0**, **BIT 1** и **BIT 2**, а факт приема - переходом пина **STROBE** в состояние цифрового нуля.

Так как есть три _информационных_ пина (**BIT 0**, **BIT 1** и **BIT 2**) и у каждого может быть два цифровых состояния, то всего можно передать **8** различных кодов.

Более того, плата передатчика может работать в качестве пингера трекинговой системы [WAYU](https://docs.unavlab.com/navigation_and_tracking_systems_ru.html#wayu), предоставляя таким образом возможность обеспечения не только обратной связи, но и определения географических координат носителя в погруженном состоянии.
Данная функция активируется пином **W_PING**. При изменении состояния пина **W_PING** на активное, модуль передатчика излучает сигнал, который может быть принят навигационными [буями](https://docs.unavlab.com/documentation/RU/RWLT/RWLT_GIB_Specification_ru.html) системы [WAYU](https://docs.unavlab.com/navigation_and_tracking_systems_ru.html#wayu).
...
Для передачи одно из четырех кодов, при помощи пинов **3**, **5** и **7** (**BIT 0**, **BIT 1** и **BIT 2** соответственно) формируется передаваемый код. Соответствие номера кода и состояние пинов отражено в таблице 1.

При приеме кода модулем приемника, он, при помощи пинов **3**, **5** и **7** (**BIT 0**, **BIT 1** и **BIT 2** соответственно) формирует принятый код и переводит пин **1** (**STROBE**) в состояние цифрового нуля на 100 миллисекунд. Таким образом, подключенная к нему система должна считать состояние пинов **3**, **5** и **7** после перехода пина **1** в состояние цифрового нуля.

#### Таблица 1 - Соответствие состояние пинов и передаваемых/принимаемых кодов

| **BIT 0** (pin 3) | **BIT 1** (pin 5) | **BIT 2** (pin 7) | Передаваемый код |
| :---: | :---: | :---: | :---: |
| 0 | 0 | 0 | 0 | 
| 1 | 0 | 0 | 1 |
| 0 | 1 | 0 | 2 |
...
В режиме пингера [WAYU](https://docs.unavlab.com/navigation_and_tracking_systems_ru.html#wayu) минимальное время между излучением сигналов составляет 2 секунды (2000 миллисекунд).
...
**A**: **Наиболее простой способ использовать две пары приемник-передатчик, соединив обе пары по 1 в 1 по пинам. Второй способ подразумевает использование системы [WAYU](https://docs.unavlab.com/navigation_and_tracking_systems_ru.html#wayu): на управляемом объекте располагается модуль передатчика, пины STROBE приемника и W_PING передатчика соединяются. Таким образом при приеме любого управляющего сигнала, управляемый объект излучит сигнал пингера навигационной системы [WAYU](https://docs.unavlab.com/navigation_and_tracking_systems_ru.html#wayu), который будет принят навигационными буями, после чего будет вычисленно географического положение управляемого объекта, что будет являтся фактом успешного приема управляющей команды.**
