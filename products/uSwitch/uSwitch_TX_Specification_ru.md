...
* **Передача до 8 кодов управления (4 в текущей версии, 8 планируется в следующей)**
...
## ОПИСАНИЕ
Модуль выполнен в виде печатной платы, к которой подключается гидроакустическая передающая антенна. Устройство позволяет передавать при помощи гидроакустического (ультразвукового) сигнала до 8 управляющих кодов, которые затем могут быть приняты и переданы пользователю при помощи приемного модуля [uSwitch RX](uSwitch_RX_Specification_ru) и сигнал пингера системы [WAYU](https://docs.unavlab.com/navigation_and_tracking_systems_ru.html#wayu), что позволяет определять географическое положение модуля передатчика в подводном положении.
...
| МИНИМАЛЬНЫЙ ПЕРИОД МЕЖДУ ДВУМЯ ПИНГАМИ WAYU | 2000 мс |
